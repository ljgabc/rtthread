...
#if defined(CONFIG_N32G003) && defined(CONFIG_USE_ADC)

#include "common/kk_dev.h"
#include "hal/kk_adc.h"
#include "mcu/n32g003/library/n32g003_adc.h"
 ...
 * @return 引脚描述结构体
 */
KK_INLINE struct adc_desc* _get_adc(int32_t channel)
{
    if (channel == 0 || channel >= kk_length_of(adcs)) {
        return NULL;
    }
...
static kk_err_t _n32g003_adc_open(const struct kk_dev* dev, int32_t channel, int32_t mode)
{
 ...
 * @return ADC转换值
 */
static uint32_t _n32g003_adc_read(const struct kk_dev* dev, int32_t channel)
{
    const struct adc_desc* adc = _get_adc(channel);
    if (!adc) {
        return KK_INVALID;
...
static struct kk_adc_dev _n32g003_adc_dev = {
    .open = _n32g003_adc_open,
    .read = _n32g003_adc_read,
};

KK_INLINE void _n32g003_adc_init(void)
{
    device_register(&_n32g003_adc_dev.dev, "ADC");
}
KK_INIT_PLATFORM_EXPORT(_n32g003_adc_init);

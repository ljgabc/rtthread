...
void I2C_Clock_Speed_Config(I2C_Module* I2Cx, uint32_t clk_speed, uint16_t duty_cycle)
{
    ...
    pclk_value = rcc_clocks.PclkFreq;
    /* Set frequency bits depending on pclk_value value */
    freq_value = ( uint16_t )( pclk_value / 1000000 );
	if (freq_value > APB_FREQ_MAX_VALUE) 
    {
        freq_value = APB_FREQ_MAX_VALUE;
    }
    ...
    temp_value = 0;
		 ...
		 /*If the ClkSpeed does not exceed 100KHz, mode(FSMODE[bit15]) is standard, duty is fixed at 1:1,ignore
		 DUTY[bit14] */
    ...
    if ( clk_speed <= CLK_SPEED_100K )
    {
		...
		/*If ClkSpeed exceeds 100KHz, mode(FSMODE[bit15]) is fast, duty(DUTY[bit14]) can be
		configured as 2:1 or 16:9 */
    /* Configure speed in fast mode */
    else if ( clk_speed <= CLK_SPEED_1M )
    {
        ...
        else /*I2C_InitStruct->DutyCycle == DutyCycle_16_9*/
        {
        ...
        /* if APB frequency is 48MHz,period = 1/48*1000ns,so trise_value = period/TRISE+1=48*TRISE/1000+1,See the manual for details*/
...
*\*\          - I2C_REG_GFLTRCTRL GFLTRCTRL register
*\*\          - I2C_REG_BYTENUM BYTENUM register
*\*\return  none
**/
...
*\*\            Where x is the peripheral instance (I2C, I2C2 ...)
...
*\*\         library (n32g003_i2c.h) or to custom values defined by user.
...
*\*\        I2C_Events in n32g003_i2c.h file.
...
ErrorStatus I2C_Event_Check( I2C_Module* I2Cx, uint32_t I2C_event )
{
    ...
    le_value = (flag1 | flag2) & I2C_FLAG_Mask;
...
/**
*\*\name    I2C_ByteNum_Enable.
*\*\fun     Enables the specified I2C peripheral ByteNum function.
*\*\param   I2Cx :
*\*\          - I2C
*\*\return  none
**/
...
/**
*\*\name    I2C_ByteNum_Disable.
*\*\fun     disables the specified I2C peripheral ByteNum function.
*\*\param   I2Cx :
*\*\          - I2C
*\*\return  none
**/
...
/**
*\*\name    I2C_Master_Received_DataBytesNum_Set.
...
/**
*\*\name    I2C_ByteNum_Last_StartStop_Set.
